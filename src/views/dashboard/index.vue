        ...
        <div class="panel">
          ...
          <div class="todo-list">
            <div class="todo-item">
              <span>组织总人数</span>
              <!-- 起始值 终点值  滚动时间 
                有冒号:代表他是个数字/变量，1000代表1000ms=1s，从0滚动到228  -->
              <count-to 
              :start-val="0"
          ...
          <div class="information-list">
            <div v-for="( item,index) in list" :key="index" class="information-list-item">
              ...
              <div>
                ...
                <!--文本插值{{变量名}} 或者 <p v-text="item.createTime"></p> -->
                <p>{{item.createTime}}</p>
              </div>
            </div>
...
import { LineChart } from 'echarts/charts'; //引入图表类型——折线图
...
export default {
  ...
  //计算属性
  computed:{
    ...mapGetters(['name','avatar','company','departmentaname'])
  },
//用watch监听homeData的变化
watch:{
  homeData(){
  ...
  mounted(){
    //获取展示的数据 设置给突变
    //用watch监听homeData的变化，一开始为空，获取数据后就初始化图表
    //this.social属性没有在data中声明，该属性不是响应式（只需要传给图表就行，不需要响应式）
    this.social = echarts.init(this.$refs.social)// 初始化echart
    this.provident = echarts.init(this.$refs.provident)// 初始化echart
  },
  methods:{
    async getHomeData(){
